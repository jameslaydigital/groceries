<script>
  import { signup } from '../lib/store.svelte.js'
  ...
  $effect(() => {
    if (!checked && token) {
      checked = true
      rpc('auth.inviteInfo', { token })
        .then((res) => {
          if (res?.valid) info = res
          else error = 'This invite is invalid or has already been used.'
        })
        .catch((e) => {
          error = e.message || 'Could not check this invite'
        })
    }
  })

  $effect(() => {
    if (info && emailField) emailField.focus()
  })
...
<div class="auth">
  ...
  <div class="card">
    <div class="brand">
      ...
      <h1>You're invited to Syncart</h1>
      {#if info}
        <p class="family">{info.family.name}</p>
        <p class="hint">Join the {info.family.name} grocery list{info.email ? ` (invited as ${info.email})` : ''}.</p>
      {:else if !error}
        <p class="family">Checking your invite…</p>
      {/if}
    </div>
    ...
    {:else if info}
      <form onsubmit={(e) => { e.preventDefault(); submit() }}>
        <label class="field">
          <span>Your name</span>
          <input bind:value={name} placeholder="e.g. Jane" autocomplete="name" />
        </label>
        <label class="field">
          <span>Email</span>
